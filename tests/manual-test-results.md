...
| TC-04 | Optional Field Defaults | Menu renders with default branding, no errors | Menu displayed "KBU Workstation Deployment Tool v1.2.0" in yellow. Background dark blue, foreground white. No errors. | ✅ PASS |
...
| TC-14 | Log Generation | File created, UTF-8, all entries present | `kurulum_log.txt` created on Desktop (16 KB). Header: app name, version `v1.2.0`, timestamp `30.06.2026 - 16:15:37`, USB path `D:\`. Six `[OK]` entries for offline deployment. Footer with end timestamp. UTF-8 verified (Turkish characters intact). | ✅ PASS |
